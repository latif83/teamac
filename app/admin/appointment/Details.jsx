...
export const AppointmentDetails = ({ appointment, setViewAppointment,setFetchAppointment }) => {
    ...
    const handleWhatsApp = () => {
        ...
        const number = `${appointmentData.phone}`.replace(/\+/g, '');

        const message = `Hello ${appointmentData.name},\n\nI am contacting you regarding your appointment.`;
            ...
            <div className="bg-white rounded-t-lg overflow-y-auto shadow-lg w-full h-full max-w-3xl relative md:p-6 p-3">
                ...
                <div className="mt-8 grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-700">{appointmentData.name}</p>
                    </div>

                    <div>
                        <p className="font-bold text-red-600 text-xs">Email</p>
                        <p className="text-sm text-gray-700">{appointmentData.email}</p>
                    </div>

                    <div>
                        <p className="font-bold text-red-600 text-xs">Phone</p>
                        <p className="text-sm text-gray-700">{appointmentData.phone}</p>
                    </div>

                    <div>
                        <p className="font-bold text-red-600 text-xs">Country</p>
                        <p className="text-sm text-gray-700">{appointmentData.countryName}</p>
                    </div>

                    <div>
                        <p className="font-bold text-red-600 text-xs">Mode</p>
                        <p className="text-sm text-gray-700">{appointmentData.mode}</p>
                    </div>

                    <div>
                        <p className="font-bold text-red-600 text-xs">Status</p>
                        <p className={`text-sm ${appointmentData.status == "Pending" ? 'text-orange-700' : 'text-green-700'}`}>{appointmentData.status}</p>
                    </div>

                    <div className="md:col-span-2">
                        <p className="font-bold text-red-600 text-xs">Additional Info</p>
                        <p className="text-sm text-gray-700">{appointmentData.message || "N/A"}</p>
                    </div>
                ...
                <div className="mt-4 grid md:grid-cols-2 gap-4">
                  ...
                  <Link
                        href={`tel:${appointmentData.phone}`}
                        className="text-sm flex items-center gap-2 bg-gray-900 hover:opacity-80 cursor-pointer text-white py-2 px-4 rounded-lg justify-center"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                ...
                <div className="mt-4">
                    ...
                    <div className="mt-1 grid md:grid-cols-2 gap-4">
                        ...
                        <button onClick={handleWhatsApp} type="button" className="text-sm flex items-center gap-2 bg-gray-100 hover:opacity-80 cursor-pointer py-2 px-4 rounded-lg justify-center border">
